...
class XEntityPage(override val context: HttpContext) : OnHttpContext {
    val fixWhere: Where = XGroup::eid EQ 0
    ...
    fun update(id: Long, column: String, value: String): JsonResult {
        ...
        if (!r.OK) return r
        val e: XGroup? = SELECT(XGroup.ALL, "p.name" AS XGroup::parentName.userName).FROM(
            XGroup LEFT_JOIN (XGroup AS "p") ON (XGroup::pid EQUAL "p.id")
        ).WHERE(XGroup::id EQ id).LIMIT(1).query<XGroup>().oneOrm()
        return e?.jsonResult(includes = listOf(XGroup::parentName)) ?: BadValue
    }
    ...
    @Action
    fun create(name: String, pid: Long, type: Int): JsonResult {
        val e = XGroup.createEntity(name, pid, type) ?: return BadValue
        return e.jsonResult()
    }
    ...
    fun list(): JsonResult {
        ...
        val ls: List<XGroup> = SELECT(XGroup.ALL, "p.name" AS XGroup::parentName.userName).FROM(
            XGroup LEFT_JOIN (XGroup AS "p") ON (XGroup::pid EQUAL "p.id")
        ).WHERE(w).orderByCTX().limitByCTX().query<XGroup>().listOrm()
        val total = XGroup.countAll(w)
        return ls.jsonResult(total = total, offset = offsetValue, includes = listOf(XGroup::parentName))
    }
}